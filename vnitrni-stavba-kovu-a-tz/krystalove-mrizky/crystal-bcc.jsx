// crystal-bcc.jsx — 3D assembly of a body-centred cubic (BCC) unit cell.
// Same look & pacing as the HCP / FCC pieces.
...
// single body-centre atom
const BODY = [0, 0, 0];
const bodyAppear = 3.5;

const ATOMS = [];
CORNERS.forEach((p, i) => ATOMS.push({ p, layer: 'A', appear: cornerAppear(i) }));
ATOMS.push({ p: BODY, layer: 'B', appear: bodyAppear });

// Cube frame: the 12 edges, grown as corners land.
const CUBE_EDGES = [];
for (let i = 0; i < 8; i++) {
  for (let j = i + 1; j < 8; j++) {
    let diff = 0;
...
// Body bonds: centre to each of the 8 corners (nearest neighbours, a√3/2). Grow one by one.
const BONDS = CORNERS.map((cn, i) => ({ from: BODY, to: cn, appear: bodyAppear + 0.15 + i * 0.12 }));
...
function Scene() {
  ...
  ATOMS.forEach((a) => {
    ...
    const r = (a.layer === 'B' ? 34 : 30) * sc * (1 + pr.depth * 0.05);
  ...
  const steps = [
    { n: '01', txt: '8 atomů ve vrcholech krychle', a: 1.2, b: 1.7, c: 3.2, d: 3.6 },
    { n: '02', txt: 'Jeden atom v tělesovém středu', a: 3.5, b: 4.0, c: 4.9, d: 5.3 },
    { n: '—',  txt: 'Krychlová prostorově středěná · BCC', a: 5.4, b: 5.9, c: 7.6, d: 8.0 },
  ];

  const metals = [
    { s: 'Fe', name: 'Železo α', part: 'Konstrukční oceli' },
    { s: 'W',  name: 'Wolfram',  part: 'Žhavicí vlákna a nástroje' },
    { s: 'Cr', name: 'Chrom',    part: 'Tvrdé pochromování' },
    { s: 'Mo', name: 'Molybden', part: 'Legura žárupevných ocelí' },
    { s: 'V',  name: 'Vanad',    part: 'Pružinové a nástrojové oceli' },
  ];
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'absolute', left: 84, top: 74, opacity: fade(tc, 0.2, 0.9, 20, 21) }}>
        ...
        <div style={{ fontFamily: sans, fontSize: 40, fontWeight: 600, color: '#eaf2fa', marginTop: 10, letterSpacing: '-0.01em' }}>Krychlová prostorově středěná – BCC</div>
      </div>

      <div style={{ position: 'absolute', left: 84, bottom: 76, display: 'flex', gap: 26,
        fontFamily: mono, fontSize: 17, color: '#aebfcf', opacity: fade(tc, 3.4, 4.0, 8.2, 8.6) }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ width: 15, height: 15, borderRadius: '50%', background: '#5cb0e6' }} />vrcholy
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ width: 15, height: 15, borderRadius: '50%', background: '#efab54' }} />tělesový střed
        </span>
      </div>
...
function BccAnimation() {
  return (
    <Stage width={1920} height={1080} duration={28} background="#080b12" persistKey="bcc-crystal" loop={false}>
      <Scene />
    </Stage>
  );
}

window.BccAnimation = BccAnimation;
